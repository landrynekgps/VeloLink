...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if connection_type == CONN_TYPE_SERIAL:
        # Serial connection
        if port1 := entry.data.get(CONF_PORT1):
        ...
        if port2 := entry.data.get(CONF_PORT2):
    ...
    # Register services
    async def handle_discovery_bus1(call: ServiceCall) -> None:
        """Handle discovery for bus1."""
        await hub.async_discovery_bus("bus1")

    async def handle_discovery_bus2(call: ServiceCall) -> None:
        """Handle discovery for bus2."""
        await hub.async_discovery_bus("bus2")

    async def handle_discovery_all(call: ServiceCall) -> None:
